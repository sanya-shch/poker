...
const CardItem = ({ cardId }) => {
  ...
  return cardId ? (
    <>
      <div className="card_item desktop_show">
        <p>{cardNumber}</p>
        {iconsData === "b" /*|| window.innerWidth <= 1279*/ ? (
          <img src={cardTypeIcons[`${cardType}Big`]} alt="" />
        ) : (
          iconsData.map((iconsRow, index) => (
            <div
              className="icons_row"
              key={`card-row-${cardType}-${cardNumber}-${index}`}
            >
      ...
      <div className="card_item desktop_hide">
        <p>{cardNumber}</p>
        <img src={cardTypeIcons[`${cardType}Big`]} alt="" />
        <p>{cardNumber}</p>
      </div>
